...
- **창을 숨긴 상태에서도 임시파일 정리** — 고급 메뉴의 **PC Manager 백그라운드 연동 (실험)**을 켜면 PC Manager 자체 검사·메모리 정리·임시파일 삭제를 순서대로 요청합니다. 지원 버전에서는 매번 홈 창을 열 필요가 없습니다.
- **정리 완료 확인·수치 갱신** — 내부 정리 완료 상태를 확인한 경우에만 완료를 표시합니다. 조회와 정리의 중복 실행을 막고, 응답이 불확실한 부스트를 자동 반복하지 않습니다.
- **지원 버전과 연결 관리** — Microsoft PC Manager Store판 **3.22.4.0 전용**, 기본 꺼짐입니다. 옵션을 켜면 PC Manager가 로컬 진단 연결로 재시작됩니다. 다른 로컬 프로세스가 접근할 수 있는 진단 포트를 사용하며, 옵션을 끄면 일반 실행으로 복원합니다. PC Manager 종료 후에는 다음 조회에서 재연결합니다. 업데이트로 버전이 바뀌면 내부 연결을 중단합니다.
- **네이티브 개발·배포 정리** — 사용하지 않는 Electron 실행 코드와 검증 도구를 정리하고 Rust 앱 중심으로 실행·테스트·설치 문서를 갱신했습니다. 기존 설치 위치와 설정을 유지합니다.

### 0.8.7

- **메모리 옆 임시파일 용량** — PC Manager 홈 화면의 임시파일 수치를 30초마다 표시합니다. 값을 읽지 못하면 0으로 표시하지 않고 `확인 불가`로 안내합니다. 몽글이 직접 디스크를 검사하거나 파일을 삭제하지 않습니다.
- **작고 정돈된 내 PC 카드** — 메모리·임시파일을 한 줄에 정렬하고, CPU는 얇은 그래프로 표시합니다. 기본 크기는 224×141이며 기존 PC Manager 부스트 버튼과 크기·위치 설정을 유지합니다.
- **부스트 뒤 수치 갱신** — 부스트 요청 시 이전 임시파일 수치를 비우고 응답 5초 뒤 다시 읽습니다. 요청 전에 시작된 조회 결과가 나중에 도착해도 이전 수치를 되살리지 않습니다.

### 0.8.6

- **부스트 준비 안내에서 PC Manager 열기** — PC Manager가 꺼져 있거나 부스트 버튼이 준비되지 않으면, 사라지지 않는 안내와 **PC Manager 열기** 버튼을 표시합니다. 홈 화면이 준비되면 몽글에서 부스트를 다시 눌러 주세요. 초기 설정이 나오면 먼저 완료해 주세요.
- **홈 화면 로딩 대기** — 부스트 버튼 조회를 최대 5초 동안 다시 시도합니다. 실제 부스트 호출은 한 번만 하며, 응답 시간 초과 시 자동으로 재실행하지 않습니다.
- **X로 닫은 뒤 사용 안내** — PC Manager 3.22.4.0에서 홈 화면을 연 뒤 X로 닫아도 동작한 사례를 확인했습니다. 앱이 종료되거나 화면 상태가 바뀌어 버튼을 찾지 못하면 다시 열기가 필요할 수 있습니다. 단순히 프로세스가 실행 중이라는 이유만으로 부스트 가능 여부를 보장하지 않습니다.
- **기존 업데이트 흐름 유지** — 새 버전 자동 다운로드 후 카드 또는 트레이의 **설치 후 재시작**으로 업데이트합니다. PC Manager 미설치 시 공식 Store 설치 안내도 유지합니다.
...
- **자동 업데이트 다운로드 복원** — 새 버전을 찾으면 백그라운드에서 자동으로 내려받습니다. 완료 후 카드 또는 트레이에서 **설치 후 재시작**만 누르면 됩니다. 설치는 사용자가 선택할 때만 진행됩니다.
- **설치 버튼 유지** — 다운로드 완료 카드는 직접 닫을 때까지 남습니다. `나중에`를 눌러도 트레이의 설치 버튼은 유지되며, 업데이트 재확인으로 준비된 설치 파일을 잃지 않습니다.
- **다운로드 상태·재시도 개선** — 확인 중·다운로드 중·최신 버전·오류를 메뉴에 표시합니다. 재시작 뒤 이미 받은 파일은 무결성을 확인해 재사용하고, 불완전한 파일은 다시 받습니다.
- **0.8.2·0.8.3 사용자 안내** — 기존 앱에서는 이번 한 번 `고급 → 업데이트 확인 → 업데이트 다운로드 → 설치 후 재시작`을 선택하세요. 다운로드 완료 뒤에는 재확인 대신 설치를 선택하세요. 앱 내 업데이트가 실패하면 [최신 설치 파일](https://github.com/bokjk/mongle-releases/releases/latest)을 받아 기존 설치 위치에 덮어설치하면 됩니다. 먼저 제거할 필요는 없습니다.
