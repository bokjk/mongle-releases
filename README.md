...
- **임시파일**: PC Manager 홈 화면에 표시되는 임시파일 용량을 30초마다 읽습니다. 앱이 꺼져 있거나 홈 화면을 읽을 수 없으면 `확인 불가`로 표시합니다. 부스트 요청 시 이전 값을 비우고 응답 5초 뒤 다시 확인합니다.
- **PC Manager 부스트·열기·설치 안내**: Microsoft PC Manager(Store판)가 설치돼 있으면 카드 하단에서 **PC Manager 부스트**를 누릅니다. 앱이 꺼져 있거나 버튼이 준비되지 않았다면 알림의 **PC Manager 열기**를 누르고 홈 화면이 로드된 뒤 몽글에서 부스트를 다시 눌러 주세요. 초기 설정이 표시되면 먼저 완료해 주세요. 홈 화면을 준비한 뒤 X로 닫고도 동작한 사례를 확인했지만, 앱 종료나 화면 상태 변화로 버튼을 찾지 못하면 다시 열기가 필요할 수 있습니다. 부스트 요청 자체는 창을 열지 않으며, 열기 버튼을 선택했을 때만 창을 엽니다. 미설치 시 **PC Manager 설치**가 공식 Microsoft Store 페이지를 열고, 카드 활성화 중 30초 간격으로 설치 상태를 다시 확인합니다. 몽글이 직접 파일을 삭제하거나 메모리를 정리하지 않습니다.
